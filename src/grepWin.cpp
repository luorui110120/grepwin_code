                       ...
                       int       nCmdShow)
{
    ...
    if (hWnd)
    {
        ...
        else if (bOnlyOne)
        {
			...
			{
				spath = parser.HasVal(L"searchpath") ? parser.GetVal(_T("searchpath")) : L"";
			}
			SearchReplace(spath, L"/", L"\\");
			SearchReplace(spath, L"\"", L"");
			if (!PathIsDirectory(spath.c_str()))
			{
				spath = spath.substr(0, spath.find_last_of(L"\\"));
    ...
    if (!bQuit)
    {
        ...
        else
        {
            ...
            else if (parser.HasVal(_T("searchpath")))
            {
